...
(A) 確保資料傳送之正確性 資料確認與重送
(B) 資料開始傳送時不需進行交握(Hand shaking) (UDP)
(C) 傳送發生錯誤時不會要求重新傳送 (UDP)
(D) 傳送時所進行之檢查與偵錯機制較 UDP 簡單 (複雜)
```
TCP 協定的特性
資料確認與重送
流量控制
...
此回應碼表示回傳的中介資料集與並非與原始伺服器上的有效確定集合完全相同
而是來自本地或第三方的副本。除此情況外，200 OK 回應碼應該被優先使用。
...
IPv4位址記錄
功能：
傳回一個32位元的IPv4位址，最常用於對映主機名稱到IP位址
```
TXT
```
定義:
